...
int tfs_destroy_after_all_closed() {
    ...
    if (pthread_mutex_lock(&single_global_lock) != 0) return -1;
    while (open_files) pthread_cond_wait(&cond, &single_global_lock);
    if (pthread_mutex_unlock(&single_global_lock) != 0) return -1;
    if (tfs_destroy() == -1) return -1;
...
int tfs_close(int fhandle) {
    ...
    open_files--;
    if (!open_files) pthread_cond_broadcast(&cond);
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;
